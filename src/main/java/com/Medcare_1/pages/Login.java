package com.Medcare_1.pages;

import static org.testng.Assert.assertTrue;

import java.awt.AWTException;
import java.io.File;
import java.io.IOException;
import java.text.DateFormat;
import java.text.SimpleDateFormat;
import java.util.Date;

import org.apache.commons.io.FileUtils;
import org.openqa.selenium.By;
import org.openqa.selenium.OutputType;
import org.openqa.selenium.TakesScreenshot;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.chrome.ChromeDriver;
import org.openqa.selenium.support.ui.ExpectedConditions;
import org.testng.Assert;
import org.testng.annotations.Test;



public class Login {
	WebDriver driver;

	// Finding locator for Password
	By tab1 = By.id("tab1");
	By tab2 = By.id("tab2");
	By tab3 = By.id("tab3");
	By tab4 = By.id("tab4");
	By tab5 = By.id("tab5");
	By tab6 = By.id("tab6");
	By tab7 = By.id("tab7");
	By tab8 = By.id("tab8");

	By kk = By.cssSelector("span[ng-reflect-message='Dashboard > Dash']");
	
	
	By UserName = By.cssSelector("input[ng-reflect-placeholder='User Name']");
	By Password = By.cssSelector("input[ng-reflect-placeholder='Password']");
	By Close = By.xpath("//mat-icon[contains(text(),'close')]");

	public Login(WebDriver driver) {
		this.driver = driver;
	}
	
	
public void KPMC_LOGIN(String UserID, String Passwordd) throws InterruptedException, IOException {
	
		driver.manage().window().maximize();
		Thread.sleep(2000);
		driver.findElement(UserName).sendKeys(UserID);
		Thread.sleep(2000);
		driver.findElement(Password).sendKeys(Passwordd);
		Thread.sleep(13000);
		
	}

	public void login_Medcare(String Uesrid) throws InterruptedException, IOException {
		

		driver.manage().window().maximize();
		Thread.sleep(2000);
		driver.findElement(tab1).sendKeys(Uesrid);
		driver.findElement(tab2).sendKeys(Uesrid);
		driver.findElement(tab3).sendKeys(Uesrid);
		driver.findElement(tab4).sendKeys(Uesrid);
		driver.findElement(tab5).sendKeys(Uesrid);
		driver.findElement(tab6).sendKeys(Uesrid);
		driver.findElement(tab7).sendKeys(Uesrid);
		driver.findElement(tab8).sendKeys(Uesrid);
		
		
		//Assert(false);
		
		Thread.sleep(15000);
		
		driver.findElement(Close).click();
	
		
	}
	
	
	
	
public void login_Medcare_1() throws InterruptedException, IOException, AWTException {
		

		driver.manage().window().maximize();
		Thread.sleep(2000);
		driver.findElement(tab1).sendKeys("1");
		driver.findElement(tab2).sendKeys("0");
		driver.findElement(tab3).sendKeys("0");
		driver.findElement(tab4).sendKeys("2");
		driver.findElement(tab5).sendKeys("1");
		driver.findElement(tab6).sendKeys("2");
		driver.findElement(tab7).sendKeys("3");
		driver.findElement(tab8).sendKeys("4");
		
		
		//Assert(false);
		
		Thread.sleep(13000);
		
		
		
		
	}

public void login_Medcare_2() throws InterruptedException, IOException, AWTException {
	

	driver.manage().window().maximize();
	Thread.sleep(2000);
	driver.findElement(tab1).sendKeys("1");
	driver.findElement(tab2).sendKeys("0");
	driver.findElement(tab3).sendKeys("5");
	driver.findElement(tab4).sendKeys("7");
	driver.findElement(tab5).sendKeys("1");
	driver.findElement(tab6).sendKeys("0");
	driver.findElement(tab7).sendKeys("5");
	driver.findElement(tab8).sendKeys("7");
	
	
	//Assert(false);
	
	Thread.sleep(13000);
	
	
	
	
}
public void login_Medcare_8() throws InterruptedException, IOException, AWTException {
	

	driver.manage().window().maximize();
	Thread.sleep(2000);
	driver.findElement(tab1).sendKeys("1");
	driver.findElement(tab2).sendKeys("0");
	driver.findElement(tab3).sendKeys("0");
	driver.findElement(tab4).sendKeys("2");
	driver.findElement(tab5).sendKeys("1");
	driver.findElement(tab6).sendKeys("2");
	driver.findElement(tab7).sendKeys("3");
	driver.findElement(tab8).sendKeys("4");
	
	
	//Assert(false);
	
	Thread.sleep(13000);
	
	
	
	
}

public void login_Medcare_3() throws InterruptedException, IOException, AWTException {
	

	driver.manage().window().maximize();
	Thread.sleep(2000);
	driver.findElement(tab1).sendKeys("1");
	driver.findElement(tab2).sendKeys("0");
	driver.findElement(tab3).sendKeys("7");
	driver.findElement(tab4).sendKeys("2");
	driver.findElement(tab5).sendKeys("1");
	driver.findElement(tab6).sendKeys("0");
	driver.findElement(tab7).sendKeys("7");
	driver.findElement(tab8).sendKeys("2");
	
	
	//Assert(false);
	
	Thread.sleep(13000);
	
	
	
	
}
}

	
















